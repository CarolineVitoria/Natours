...
module.exports = (err, req, res, next) => {
    ...
    if(process.env.NODE_ENV === 'development') {
      console.log('dfjhasgfhgsdhjkhsjk')
      sendErroDev(err, res);
    }else if (process.env.NODE_ENV === 'production') {
      //let error = err; //You can't clone the err-object without loosing some properties. The approach given by Jonas will not work in this way. So either you copy the name property manually to the clone, or a better solution (at least in my opinion): don't clone the err-object (we've modified it yet by adding statusCode and status).
